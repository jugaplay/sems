(function(){
  ...
  var datatables1 = $( '#datatables1' ).dataTable({
    ...
    'fnInitComplete': function(settings) {
      ...
      $.each( aoData, function(i, val){
        ...
        $ntr.attr( 'data-streetmap', val._aData[4]);
        $ntr.attr( 'data-streetid', val._aData[5]);
      });
  ...
  .on( 'submit', '#formEditDatatables1', function(e){
    ...
    var jqxhr = $.ajax({
                    ...
                    url: window.apiUrl+"blocks/"+datas[1].value,//blocks/id
                    data: datas
                  })
                  .done(function(xhr) {
                    //alert( 'S'+JSON.stringify(xhr) );
  ...
  getDataSelected = function(){
    ...
    streetid  = dataSelected.data( 'streetid' ),
    name = dataSelected.children( 'td:eq(0)' ).text(),
    maxnum = dataSelected.children( 'td:eq(1)' ).text(),
    minnum = dataSelected.children( 'td:eq(2)' ).text(),
    size = dataSelected.children( 'td:eq(3)' ).text(),
